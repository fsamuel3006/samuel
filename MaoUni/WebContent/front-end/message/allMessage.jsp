...
<!--請看這行最右邊-->  <a class="btsp" href="Item_select_page.jsp">客服中心</a>  <!-- 這行是商城管理的標題，可以自行設定，還有href可以自行設定跳轉的頁面 -->
...
<body bgcolor='white'>
	...
	<table>
		...
		<c:forEach var="MessagVO" items="${list}">

			<tr>
				<td>${MessagVO.id}</td>
				<td>${MessagVO.memId}</td>
				<td>${MessagVO.contain}</td>
				<td>${MessagVO.status}</td>
				<td>${MessagVO.time}</td>

				<td>
					<FORM METHOD="post"
						ACTION="<%=request.getContextPath()%>/messageServlet"
						style="margin-bottom: 0px;">

						<input type="hidden" name="id" value="${MessagVO.id}"> 
						<input type="hidden" name="action" value="sendMail" onclick="success"> <input
							type="submit" value="回覆">
					</FORM>
				</td>
	...
	<input type="button" value="回上一頁"
		onclick="window.location.href = '<%=request.getContextPath()%>/back-end/Staff/backImage.jsp'"></input>
